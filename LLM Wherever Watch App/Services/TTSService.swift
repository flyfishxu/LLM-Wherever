...
class TTSService: NSObject, ObservableObject, AVSpeechSynthesizerDelegate {
    ...
    /// Update settings from external source (e.g., iPhone sync)
    func updateSettings(_ newSettings: TTSSettings) {
        isUpdatingFromiPhone = true
        settings = newSettings
        hasUnsavedChanges = false
        isUpdatingFromiPhone = false
    }
    
    /// Save settings and sync to iPhone
    func saveSettings() {
        settings.save()
        hasUnsavedChanges = false
    ...
    private func cleanTextForTTS(_ text: String) -> String {
        ...
        // Limit length for watch performance (max 500 characters)
